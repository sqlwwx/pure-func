...
describe('simpleExpireStore', () => {
  ...
  it('register async', async () => {
    ...
    assert(await store.origin().random.asyncValue === await asyncValue1)
    const asyncValue2 = store.getAsync('random')
    assert(await store.origin().random.asyncValue === await asyncValue2)
    const asyncValue3 = store.getAsync('random')
    assert(await store.origin().random.asyncValue === await asyncValue3)
